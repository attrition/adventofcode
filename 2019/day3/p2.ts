...
namespace day3 {
    ...
    console.log(((wires: string[]): number => {
        ...
        wires.forEach((wire, idx) => {
            let commands = wire.split(",");
            let loc: [number, number] = [0, 0];
            let totalSteps = 0;

            commands.forEach(command => {
                let dir = command[0];
                [loc, totalSteps] = addWireToGrid(idx, loc, dir,
                    Number.parseInt(command.substring(1)),
                    totalSteps);
            });
        });
    ...
    function addWireToGrid(wireNum: number, loc: [number, number],
            dir: string, times: number, totalSteps: number)
        ...
        for (let i = 0; i < times; ++i) {
            ...
            loc[0] += step[0];
            loc[1] += step[1];
            const strLoc = loc[0] + "," + loc[1];
            if (grid[strLoc] != undefined && grid[strLoc][wireNum] != undefined) {
                grid[strLoc][wireNum] = totalSteps;
                let combinedSteps = grid[strLoc][0] + grid[strLoc][1];
                if (combinedSteps < shortestDistance) {
                    shortestDistance = combinedSteps;
                }
            } else {
                if (grid[strLoc] == undefined) {
                    grid[strLoc] = [ [0, Number.MAX_SAFE_INTEGER],
                            [1, Number.MAX_SAFE_INTEGER] ]; 
                }
                grid[strLoc][wireNum] = totalSteps;
            }
        }
        return [loc, totalSteps];
    }
}