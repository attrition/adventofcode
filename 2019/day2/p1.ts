...
console.log((function(program : number[]) {

    let offset = 0;
    while (program[offset] != 99) {
        const op = program[offset];
        const idxA = program[offset + 1];
        const idxB = program[offset + 2];
        const resultIdx = program[offset + 3];

        switch (op)
        {
            case 1:
                program[resultIdx] = program[idxA] + program[idxB];
                break;
            case 2:
                program[resultIdx] = program[idxA] * program[idxB];
                break;
