...
count = 0
for input in inputs:
    reg = re.search("(\d+)-(\d+) (\w): (\w+)", input)
    minimum = int(reg.group(1))
    maximum = int(reg.group(2))
    letter = reg.group(3)
    phrase = reg.group(4)

    removed = phrase.replace(letter, "")
    diff = len(phrase) - len(removed)
    if diff >= minimum and diff <= maximum:
        print(minimum, maximum, letter, phrase)
        count = count + 1
