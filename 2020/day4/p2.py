...
def range_rule(minimum, maximum, value):
    value = int(value)
    return value >= minimum and value <= maximum

def height_rule(value):
    if not re.match("\d+(cm|in)", value):
        return False
    if "in" in value:
        value = int(value.replace("in", ""))
        return range_rule(59, 76, value)
    elif "cm" in value:
        value = int(value.replace("cm", ""))
        return range_rule(150, 193, value)
    return False

def colour_rule(value):
    return re.match("#[0-9a-f]{6}", value)

def set_rule(set, value):
    return value in set

def digits_rule(digits, value):
    return re.match("\d{" + str(digits) + "}$", value)

def valid_field(field, value):
    rules = {
        "byr": partial(range_rule, 1920, 2002),
        "iyr": partial(range_rule, 2010, 2020),
        "eyr": partial(range_rule, 2020, 2030),
        "hgt": height_rule,
        "hcl": colour_rule,
        "ecl": partial(set_rule, ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]),
        "pid": partial(digits_rule, 9)
    }

    return rules[field](value)

def valid_passports(formatted):
    valid = 0
...
print(str(valid_passports(formatted)))